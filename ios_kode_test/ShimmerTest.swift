...
class ShimmerTest: UIViewController {
    
    let circleImage = UIView()
    let label = UILabel()
    let desc = UILabel()
    let gradientLayer = CAGradientLayer()
    let circleLayer = CAGradientLayer()
    let descLayer = CAGradientLayer()
    ...
    func layout() {
        ...
        circleImage.snp.makeConstraints { make in
            make.top.equalTo(view.layoutMarginsGuide.snp.top).offset(6)
...
extension UIColor {
    ...
    static var gradientDarkGrey: UIColor {
        return UIColor(red: 243 / 255.0, green: 243 / 255.0, blue: 246 / 255.0, alpha: 1)
    }
